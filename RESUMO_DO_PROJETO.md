...
5. **Dashboard Admin:** Criação e importação em massa (CSV com `papaparse`) finalizadas para Coordenadores, Professores, Turmas, Disciplinas e as Associações necessárias (Professor → Disciplina → Turma).
...
2. **Versionamento:** Configurar o `.gitignore`, adicionar os arquivos não monitorados ao Git e fazer o *commit* inicial de todos os componentes e APIs geradas.
3. **Deploy (Vercel):** Empurrar o código do GitHub e realizar o deploy do front-end na plataforma Vercel, adicionando nela as mesmas chaves de ambiente (`.env.local`) de Cloudinary e Railway.
